...
data["Canonical_smiles"], data["Conversion_Success"], data["Is_NaN"] = zip(
    *[canonical_smiles(smile) for smile in tqdm(data["smiles"], desc="Processing data")]
)
...
for file in os.listdir(folder_path):
    if file.endswith(".csv"):
        ...
        df["Canonical_Smiles"], df["Conversion_Success"], df["Is_NaN"] = zip(
            *[
                canonical_smiles(smile)
                for smile in tqdm(df["Smiles"], desc=f"Processing {file}")
            ]
        )
...
data_intersection = data[
    data["Canonical_smiles"].isin(combined_df_sorted["Canonical_Smiles"])
].copy()
data_intersection = data_intersection.drop(
    ["Canonical_smiles", "Conversion_Success", "Is_NaN"], axis=1
)
