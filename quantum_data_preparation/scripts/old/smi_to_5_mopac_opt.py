...
def xyz_to_atoms(xyz_path):
    ...
    for i in range(5):
        ...
        for j in range(n):
            ...
            names.append(line[0])
            xyzs += line[1:]
        myatoms = [Atom(names[k], (float(xyzs[3*k]), float(xyzs[3*k + 1]), float(xyzs[3*k + 2]))) for k in range(len(names))]
...
class MYMOPAC(MOPAC):
    ...
    def read_results(self):
        ...
        for i in range(-2, -len(lines), -1):
            if lines[i-1].find('CARTESIAN COORDINATES') != -1 and lines[i].isspace() == True:
                number = i
                names = []
                xyzs = []
                break
        for j in range(self.num_atoms):
            print('{}/{}'.format(j+1, self.num_atoms), lines[number+1+j])
            xyzs += lines[number+1+j].split()[2:]
            names.append(lines[number+1+j].split()[1])              
        myatoms = []
        i = 0
        while i < len(names):
            A = Atom(str(names[i]), (float(xyzs[3*i]), float(xyzs[3*i + 1]), float(xyzs[3*i + 2])))
            myatoms.append(A)
            i += 1
...
def mopac_opt_thermo(e):
    for i in range(5):
        ...
        optimization_label = '{}_conformation_optimized_{}'.format(e, i)
        thermo_label = '{}_conformation_thermo_{}'.format(e, i)

        atoms.calc = MYMOPAC(label=optimization_label, task='XYZ CHARGE=0 Singlet BONDS OPT AUX')
        try:
            atoms.get_potential_energy() 
            logging.info(f"Potential energy calculated successfully for {optimization_label}")
...
for idx, smi in zip(indices, smiles):
    ...
    if mol is not None:
        ...
        if generate_conformations_sdf(mol, str(sdf_file)):
            ...
            logging.error(f"Skipping molecule with index {idx} due to errors in conformation generation.")
