...
def calc_indexes(minus, zero, plus):
    ...
    if minus is None and zero is None and plus is not None:
        ...
        minus = { 'charges': [(zero_charge + plus_charge) / 2 for zero_charge, plus_charge in zip(zero['charges'], plus['charges'])] }
    
    elif minus is not None and zero is None and plus is not None:
        # Если есть -1 и +1, 0 заполняем средним значением
        zero = { 'charges': [(minus_charge + plus_charge) / 2 for minus_charge, plus_charge in zip(minus['charges'], plus['charges'])] }
    
    elif minus is not None and zero is not None and plus is None:
        # Если есть -1 и 0, +1 заполняем средним значением
        plus = { 'charges': [(minus_charge + zero_charge) / 2 for minus_charge, zero_charge in zip(minus['charges'], zero['charges'])] }
...
print('Processing complete. Data saved to molecules_combined_data.csv and molecules_rmse_data.csv.')
``