...
# Configure logging
logging.basicConfig(
    filename="generate_conformers.log",
    level=logging.INFO,
...
for idx, smiles in tqdm(
    zip(indices, smiles_list), desc="Generating conformers", total=len(smiles_list)
):
    ...
    if mol is not None:
        ...
        if generate_conformations_sdf(mol, str(sdf_file)):
            ...
            logging.error(
                f"Failed to generate conformers for SMILES {smiles} with index {idx}"
            )
    else:
