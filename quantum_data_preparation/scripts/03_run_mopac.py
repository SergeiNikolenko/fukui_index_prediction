...
def generate_conformations_sdf(mol, sdf_path):
    try:
        ...
        for id in ids:
            ...
            mol.SetProp("ENERGY", f"{energy_value:.2f}")  # Save energy value
...
class MYMOPAC(MOPAC):
    ...
    def read_results(self):
        ...
        num_atoms = int(
            [line for line in lines if "Empirical Formula" in line][0].split()[-2]
        )
        atoms = []
        for line in lines:
            if "CARTESIAN COORDINATES" in line:
                for atom_line in lines[
                    lines.index(line) + 1 : lines.index(line) + 1 + num_atoms
                ]:
...
for idx, smi in zip(indices, smiles):
    ...
    if mol is not None:
        ...
        if generate_conformations_sdf(mol, str(sdf_file)):
            ...
            logging.error(
                f"Failed to generate conformations for SMILES {smi} with index {idx}"
            )
    else:
