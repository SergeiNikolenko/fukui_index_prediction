...
def add_skipatom_features_to_dataset(
    dataset,
    ...
    try:
        ...
        model = SkipAtomInducedModel.load(
            model_path, data_path, min_count=min_count, top_n=top_n
        )
        ...
        with torch.no_grad():
            for graph in progress:
                ...
                atom_indices = [
                    model.dictionary.get(symbol, -1) for symbol in atom_symbols
                ]
                ...
                if len(atom_features) == new_graph.x.size(0):
                    ...
                    atom_features_tensor = torch.tensor(
                        atom_features_scaled, dtype=torch.float32, device=device
                    )
                    new_graph.x = torch.cat([new_graph.x, atom_features_tensor], dim=1)
                else:
                    print(
                        f"Error: Mismatch in features count. Graph features {new_graph.x.size(0)}, Atom features {len(atom_features)}"
                    )
